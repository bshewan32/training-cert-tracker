...
const CertificatesWithDashboard = ( {
  ...
  const [complianceByPosition,
  setComplianceByPosition] = useState([]);

  const [urgentActions,
  setUrgentActions] = useState([]);
  ...
  setSelectedFilterEmployee] = useState('');

  const [selectedFilterCertType,
  setSelectedFilterCertType] = useState('');


  // Selected employee's positions
  const [employeePositions,
  setEmployeePositions] = useState([]);
  ...
  const calculateDashboardStats = () =>  {
    ...
    const expiringSoon = certificates.filter(cert =>  {
      ...
      positionStats.sort((a,
      b) => a.complianceRate - b.complianceRate);

      setComplianceByPosition(positionStats.slice(0,
      5));
      ...
      .filter(cert =>  {
        ...
        .sort((a,
        b) => new Date(a.expirationDate) - new Date(b.expirationDate))
        .slice(0,
        5)
        .map(cert => ( {
              ...
              <button
                onClick= {
                  async () =>  {
                    ...
                    ? They will be excluded from compliance calculations.`))  {
  ...
  <h3>Certificate History</h3>
  {
    ...
    <p><strong>Note:</strong> This employee is archived,
  but you can still view their certificate history.</p>
</div>
)
}
...
<td>
  <button
    onClick= {
      () => handleCertificateDelete(cert._id)
    }
...

{
  ...
  <MultiPositionComplianceDashboard token= {
    token
  }
  />
</div>
...

{
  ...
  <div className="setup-tabs">
    <button
      className= {
        `tab-button $ {
          activeTab === 'bulkImport' ? 'active' : ''
        }
        `
      }

      onClick= {
        () => setActiveTab('bulkImport')
      }
    ...
    <button
      className= {
        `tab-button $ {
          activeTab === 'certificateTypes' ? 'active' : ''
        }
        `
      }
            ...
            <div className="setup-list">
              {
                ...
                <span className="employee-info">
                  ...
                  -  {
                    emp.primaryPosition?.title || (emp.positions && emp.positions.length > 0 ? emp.positions[0].title : 'No position')
                  }
  ...
  <div className="setup-list">
    ...
    <div className="button-group">
      <button
        onClick= {
          () => setSelectedPositionForRequirements(pos)
        }
...

{
  ...
  <div className="requirements-section">
    <PositionRequirements
      ...
      <button
        onClick= {
          () => setSelectedPositionForRequirements(null)
        }
...
<input
  ...
  <small className="validity-help">
    Common periods: First Aid (36 months),
    CPR (12 months),
    Safety Training (24 months)
  </small>
</div>
<div className="form-group">
...
<h4>Existing Certificate Types</h4>
{
  ...
  className="list-item cert-type-item">
  <div className="cert-type-details">
    <span className="cert-name"> {
      cert.name
    }
  </span>
  <span className="cert-validity">Valid for  {
    cert.validityPeriod
  }
months</span>
{
  cert.description && <span className="cert-description"> {
    cert.description
  }
</span>
}
...

{
  ...
  <div className="tool-tabs">
    <button
      className= {
        `tool-tab $ {
          activeExcelTool === 'exporter' ? 'active' : ''
        }
        `
      }

      onClick= {
        () => setActiveExcelTool('exporter')
      }
    ...
    <button
      className= {
        `tool-tab $ {
          activeExcelTool === 'formatter' ? 'active' : ''
        }
        `
      }

      onClick= {
        () => setActiveExcelTool('formatter')
      }
...
<div className="certificates-with-dashboard">
    ...
    <div className="metric-card primary">
      ...
      <div className= {
        `compliance-indicator $ {
          getComplianceIndicatorClass(dashboardStats.complianceRate)
        }
        `
      }
      >
  ...
  <div className="metric-subtitle">
    {
      dashboardStats.activeCertificates
    }
    of  {
...
<div className="insights-grid">
  ...
  <div className="position-stats">
    <div className= {
      `compliance-rate $ {
        getComplianceIndicatorClass(pos.complianceRate)
      }
      `
    }
    >
...
<div className="action-urgency">
  <div className= {
    `days-left $ {
      action.daysLeft <= 7 ? 'critical' : 'warning'
      }
      `
    }
    >
...
<div className="certificates-table-section">
  ...
  <div className="filter-controls">
    ...
    <select
      ...
      onChange= {
        (e) => setSelectedFilterEmployee(e.target.value)
      }
...
<select
  value= {
    selectedFilterCertType
  }

  onChange= {
    (e) => setSelectedFilterCertType(e.target.value)
  }
...

{
  ...
  <div className="employee-actions">
    <button
      ...
      onClick= {
        () => onViewEmployee && onViewEmployee(selectedFilterEmployee)
      }
...
<tbody>
  {
    ...
    .sort((a,
    b) => new Date(b.issueDate) - new Date(a.issueDate))
    .map((cert) =>  {
      ...
      if (daysUntilExpiration <= 0) statusClass = 'status-expired';
        ...
        <td> {
          getPositionTitle(cert.position)
        }
      </td>
      <td> {
        cert.certificateName || cert.certificateType
...
function App()  {
  ...
  const [selectedEmployeeForEdit,
  setSelectedEmployeeForEdit] = useState(null);

  const [selectedPositionForRequirements,
  setSelectedPositionForRequirements] = useState(null);

  const [activeExcelTool,
  setActiveExcelTool] = useState('exporter');

  const [view,
  setView] = useState('login')
  const [token,
  setToken] = useState(localStorage.getItem('authToken') || '')
  const [error,
  setError] = useState('')
  const [message,
  setMessage] = useState('')
  const [certificates,
  setCertificates] = useState([])
  const [isAdmin,
  ...
  const [activeTab,
  setActiveTab] = useState('bulkImport')
  const [employees,
  ...
  setCertificateTypes] = useState([])
  const [selectedPosition,
  setSelectedPosition] = useState('');

  const [issueDate,
  setIssueDate] = useState('');

  const [expiryDate,
  setExpiryDate] = useState('');

  const [importFile,
  setImportFile] = useState(null);

  const [adminActiveTab,
  setAdminActiveTab] = useState('overview');

  const [showArchivedEmployees,
  setShowArchivedEmployees] = useState(false);
  ...
  type) =>  {
    ...
    try  {
      ...
      const response = await fetch(`https://training-cert-tracker.onrender.com/api/users/$ {
        type
      }
      `,
      {
  ...
  const sendReminder = async (certificateId) =>  {
    ...
    try  {
      ...
      const response = await fetch(`https://training-cert-tracker.onrender.com/api/admin/send-reminder/$ {
        certificateId
      }
      `,
      {
  ...
  useEffect(() =>  {
    ...
    if (token)  {
      ...
      if (view === 'certificates' || view === 'setup')  {
        ...
        fetchSetupData(true);
        // Always load archived employees for access
  ...
  const fetchSetupData = async (includeInactive = false) =>  {
    ...
    try  {
      ...
      const response = await fetch(`https://training-cert-tracker.onrender.com/api/setup?includeInactive=$ {
        includeInactive
      }
      `,
      {
      ...
      console.error('Setup data fetch error:',
      err);
  ...
  const handleEmployeeSubmit = async (e) =>  {
    ...
    try  {
      ...
      console.error('Employee submit error:',
      err);
  ...
  const handlePositionSubmit = async (e) =>  {
    ...
    try  {
      ...
      // <-- This is the issue - missing opening brace
        setError(err.message || 'Error adding certificate');
...

    {
      ...
      <div className="admin-tabs">
        <button
          className= {
            `tab-button $ {
              adminActiveTab === 'overview' ? 'active' : ''
            }
            `
          }
        ...
        <button
          className= {
            `tab-button $ {
              adminActiveTab === 'positions' ? 'active' : ''
            }
            `
          }

          onClick= {
            () => setAdminActiveTab('positions')
          }
        ...
        <button
          className= {
            `tab-button $ {
              adminActiveTab === 'compliance' ? 'active' : ''
            }
            `
          }

          onClick= {
            () => setAdminActiveTab('compliance')
          }
        ...
        <button
          className= {
            `tab-button $ {
              adminActiveTab === 'certificates' ? 'active' : ''
            }
            `
          }

          onClick= {
            () => setAdminActiveTab('certificates')
          }
...

{
  ...
  <EmployeePositionsDashboard token= {
    token
  }
  />
  )
...
<label>Position:</label>
{
  ...
  <option value="">Select Position</option>
  {
    ...
    {
      pos._id === (selectedEmployee.primaryPosition?._id || selectedEmployee.primaryPosition) ? '(Primary)' : ''
    }
...
<input
  ...
  name="position"
  value= {
    selectedEmployee.positions[0].title
  }
  ...
  <input
    ...
    name="position"
    value= {
      selectedPosition ? positions.find(p => p._id === selectedPosition)?.title || '' : 'Select staff member first'
    }
  ...
  onChange= {
    (e) =>  {
      ...
      if (certType && issueDate)  {
        ...
        console.log('Certificate type selected,
        waiting for issue date');
...
<input
  ...
  onChange= {
    (e) =>  {
      ...
      e.target.value);
      // Debug log
      console.log('Selected cert type for calculation:',
      selectedCertType);
      // Debug log
      ...
      if (selectedCertType && e.target.value)  {
        ...
        console.log(`Calculated expiry: $ {
          selectedCertType.validityPeriod || 12
        }
        months from $ {
          e.target.value
        }
...
<tbody>
  {
    ...
    .map((cert) =>  {
      ...
      if (daysUntilExpiration <= 0) statusClass = 'status-expired'
        ...
        <td> {
          position ? position.title : cert.position
        }
      </td>
      <td> {
        cert.certificateName || cert.certificateType
