...
const CertificatesWithDashboard = ({ 
  ...
  // Memoize active employees for performance
  ...
  useEffect(() => {
    if (formData.staffMember) {
      ...
      setEmployeePositions([]);
      // Reset position when no employee is selected
      setFormData(prev => ({
        ...prev,
        position: ''
      }));
    }
  }, [formData.staffMember, employees, positions, formData.position]);
  ...
  useEffect(() => {
    if (formData.certificateType && formData.issueDate) {
      ...
      if (selectedCertType?.validityPeriod) {
        try {
          const issueDate = new Date(formData.issueDate);
          if (isNaN(issueDate.getTime())) {
            console.error('Invalid issue date');
            return;
          }
          ...
          console.error('Error calculating expiration date:', err);
        }
      }
    }
  ...
  // Memoize filtered certificates for performance
  ...
  }, [certificates, selectedFilterEmployee, selectedFilterCertType, selectedFilterPosition]);
            ...
            {complianceByPosition.length === 0 ? (
              ...
              <div className="position-list">
                {complianceByPosition.map((pos, index) => (
                  <div key={`position-${index}`} className="position-item">
              ...
              >
                ...
                {employeePositions.map(pos => {
                  ...
                  const primaryPosId = typeof selectedEmployee?.primaryPosition === 'object' 
                    ? selectedEmployee.primaryPosition._id 
                    : selectedEmployee?.primaryPosition;
                  const isPrimary = primaryPosId === pos._id;
            ...
            <tbody>
              {filteredCertificates.map((cert) => {
                ...
                try {
                  ...
                  console.error('Error calculating expiration:', err);
                }
