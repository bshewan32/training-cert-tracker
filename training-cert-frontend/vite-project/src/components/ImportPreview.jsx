...
const ImportPreview = ({ data, onConfirm, onCancel }) => {
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      // Process the preview data to ensure dates are properly formatted
      const processedData = data.slice(0, 10).map(row => ({
        ...row,
        // Ensure dates are displayed in DD/MM/YYYY format
        'Booking Date': row['Booking Date'] || (row.parsedBookingDate ? formatDate(row.parsedBookingDate) : ''),
        'Expiry Date': row['Expiry Date'] || (row.parsedExpiryDate ? formatDate(row.parsedExpiryDate) : 'Auto-calculated')
      }));
  ...
  return (
    <div className="import-preview">
      ...
      <div className="preview-note">
        <p><strong>Date Format:</strong> All dates are displayed in DD/MM/YYYY format and will be imported correctly.</p>
      </div>
