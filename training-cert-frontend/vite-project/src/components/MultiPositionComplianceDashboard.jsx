...
const MultiPositionComplianceDashboard = ({ token }) => {
  ...
  useEffect(() => {
    fetchData();
  }, [token, includeInactiveEmployees]);
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch setup data with optional inactive employees
      const setupResponse = await fetch(`https://training-cert-tracker.onrender.com/api/setup?includeInactive=${includeInactiveEmployees}`, {
        headers: {
      ...
      const archivedResponse = await fetch('https://training-cert-tracker.onrender.com/api/setup/archived-employees', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (archivedResponse.ok) {
        ...
        setArchivedEmployees(archivedData.employees || []);
      }
  ...
  const archiveEmployee = async (employeeId) => {
    try {
      const response = await fetch(`https://training-cert-tracker.onrender.com/api/setup/employee/${employeeId}/archive`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      setError(err.message || 'Error archiving employee');
    }
  };
  ...
  const reactivateEmployee = async (employeeId) => {
    try {
      const response = await fetch(`https://training-cert-tracker.onrender.com/api/setup/employee/${employeeId}/reactivate`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      setError(err.message || 'Error reactivating employee');
    }
  };
    ...
    <div className="multi-position-compliance-dashboard">
        ...
        <div className="dashboard-controls">
          <div className="employee-filter-controls">
            <label className="checkbox-label">
              ...
              />
              Include archived employees in compliance calculations
            </label>
          </div>
          ...
          {archivedEmployees.length > 0 && (
            ...
            >
              {showArchivedView ? 'Hide' : 'Show'} Archived Employees ({archivedEmployees.length})
            </button>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {showArchivedView && (
            <div className="archived-employees-section">
              <h3>Archived Employees</h3>
              <div className="archived-employees-grid">
                {archivedEmployees.map(employee => (
                  <div key={employee._id} className="archived-employee-card">
                    <div className="archived-employee-info">
                      <div className="employee-name">{employee.name}</div>
                      <div className="employee-positions">
                        {employee.positions && employee.positions.length > 0 ? (
                          employee.positions.map(pos => (
                            <span key={pos._id} className="position-badge archived">
                              {pos.title}
                            </span>
                          ))
                        ) : (
                          <span className="no-positions">No positions</span>
                        )}
                      </div>
                      <div className="employee-email">{employee.email || 'No email'}</div>
                    </div>
                    <div className="archived-employee-actions">
                      <button
                        className="reactivate-btn"
                        onClick={() => reactivateEmployee(employee._id)}
                      >
                        Reactivate
                      </button>
                    </div>
                ...
                <div className="chart-text">
                  ...
                  <div className="chart-label">
                    ...
                    {!includeInactiveEmployees && (
                      <div className="chart-subtitle">(Active employees only)</div>
                    )}
                  </div>
                </div>
              </div>
            ...
            {filteredData.length === 0 ? (
              ...
              <div className="compliance-cards">
                {filteredData.map((data, index) => (
                  <div key={`${data.employeeId}-${data.positionId}`} className={`compliance-card ${!data.employeeActive ? 'inactive-employee' : ''}`}>
                    <div className="card-header">
                      <div className="header-main">
                        <div className="employee-name">
                          {data.employeeName}
                          {!data.employeeActive && (
                            <span className="inactive-badge">Archived</span>
                          )}
                        </div>
                      ...
                      <div className="employee-actions">
                        ...
                        {data.employeeActive && (
                          <button
                            ...
                            onClick={() => archiveEmployee(data.employeeId)}
      ...
      <style jsx>{`
        ...
        .archived-employee-info {
          flex: 1;
        }
        
        .archived-employee-info .employee-name {
          font-weight: 600;
          color: #2d3748;
          margin-bottom: 8px;
        }
